    ...
    <PageShell>
      ...
      >
        <p className="mt-md flex flex-wrap items-center gap-md text-[14px] text-on-primary-muted">
          ...
          <span className="flex items-center gap-1"><Clock size={16} className="text-accent-bright" aria-hidden /> {cfg.pickupWindow}</span>
          <span className="text-on-primary-faint">·</span>
          <span className="flex items-center gap-1"><Car size={16} className="text-accent-bright" aria-hidden /> {cfg.driveTime} drive</span>
        </p>
      ...
      <section className="mx-auto max-w-content px-5 md:px-6 mt-3xl">
        <h2 className="text-[28px] md:text-[32px] mb-sm flex items-center gap-sm">
          <Car size={28} className="text-accent-bright" aria-hidden /> Travel times from {cfg.area}
        </h2>
          ...
          <table className="w-full text-[14px] min-w-[520px]">
            ...
            <tbody>
              {cfg.travel.map((row, i) => (
                <tr key={row.from} className={i % 2 ? "bg-white/[0.02]" : ""}>
                  <td className="px-md py-sm font-semibold">{row.from}</td>
                  <td className="px-md py-sm mono text-accent-bright">{row.drive}</td>
                  <td className="px-md py-sm mono text-on-primary-muted">{row.pickup}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="text-[14px] text-on-primary-muted mt-sm">
          {cfg.metroNote}
        </p>
      </section>
      ...
      <section className="bg-white/[0.02] border-y border-white/10 mt-3xl py-3xl">
        <div className="mx-auto max-w-content px-5 md:px-6">
          ...
          <p className="text-[16px] text-on-primary-muted leading-relaxed max-w-[72ch] mb-lg">{cfg.pickupNote}</p>
        ...
        <div className="grid gap-md md:grid-cols-2 lg:grid-cols-3">
          {cfg.topServices.map((s) => (
            <Link
              key={s.href}
              to={s.href}
              className="block border border-white/10 bg-white/[0.04] rounded-md p-lg hover:border-accent-bright/40 transition"
            >
      ...
      <section className="mx-auto max-w-content px-5 md:px-6 mt-3xl">
        <h2 className="text-[28px] md:text-[32px] mb-md flex items-center gap-sm">
          <Navigation size={28} className="text-accent-bright" aria-hidden /> Landmarks we collect from in {cfg.area}
        </h2>
      ...
      <section className="mx-auto max-w-content px-5 md:px-6 mt-3xl">
        <h2 className="text-[28px] md:text-[32px] mb-md flex items-center gap-sm">
          <MapPin size={28} className="text-accent-bright" aria-hidden /> Visit our workshop or use free pickup
        </h2>
        <p className="text-[16px] text-on-primary-muted max-w-[72ch] mb-lg">
      ...
      <section className="mx-auto max-w-content px-5 md:px-6 mt-3xl">
        <article className="border border-white/10 bg-white/[0.04] rounded-md p-lg flex items-start gap-md">
          <ParkingCircle size={24} className="text-accent-bright shrink-0 mt-1" aria-hidden />
          <p className="text-[15px] text-on-primary-muted leading-relaxed m-0">
            <strong className="text-on-primary">Prefer to drop in?</strong> Paid tower parking (AED 5/hour, AED 25/day cap) or free street parking on Al Sufouh Road, 120 m from Media City Metro on the Red Line. Quietest on weekday afternoons - WhatsApp ahead at peak times for a guaranteed slot.
          </p>
        </article>
      </section>
      ...
      <section className="mx-auto max-w-content px-5 md:px-6 mt-3xl">
        ...
        <div className="flex flex-wrap gap-sm">
          {AREAS.filter((a) => a.slug !== cfg.slug).map((a) => (
            <Link key={a.slug} to={`/macbook-repair-${a.slug}`} className="rounded-full border border-white/12 bg-white/[0.04] px-4 py-2 text-[14px] text-on-primary-muted transition-colors hover:border-accent-bright/40 hover:text-on-primary">
      ...
      <section className="mx-auto max-w-content px-5 md:px-6 mt-3xl">
        <div className="relative overflow-hidden border border-white/10 bg-white/[0.04] text-on-primary rounded-2xl p-xl md:p-2xl flex flex-col items-start gap-md">
          ...
          <h2 className="relative text-on-primary text-[28px] md:text-[32px] max-w-[28ch]">Book a free pickup from {cfg.area} in 30 seconds</h2>
