...
export function WarrantyBar({ tone = "light" }: { tone?: "light" | "dark" } = {}) {
  ...
  return (
    <Reveal className={cn("border-y", dark ? "border border-white/10 bg-white/[0.04]" : "bg-success/10 border-success/30")}>
      <ul className="mx-auto max-w-content px-5 md:px-6 py-md flex flex-wrap items-center justify-center gap-x-xl gap-y-sm">
        {items.map(({ icon: Icon, label }) => (
          <li key={label} className={cn("flex items-center gap-sm text-[14px] font-medium", dark ? "text-on-primary" : "text-text")}>
