"use client";
// Shared layout for Mac Pro generation hub pages (Intel, M2 Ultra).
// Pro tower: PCIe expansion (Intel), M2 Ultra logic board, GPU modules, data recovery focus.
import {
...
export type MacProGenModel = { label: string; href?: string; aNum?: string };
export type MacProGenFAQ = { q: string; a: string };

export type MacProGenerationProps = {
  path: string;
  chip: string;        // "M2 Ultra", "Intel"
  years: string;       // "2019" | "2023"
  seoTitle: string;
  ...
  heroMeta: string[];
  models: MacProGenModel[];
  faqs: MacProGenFAQ[];
};

const HERO_IMG = "/images/real/lab/macbook-battery-internals-dubai.jpg";
const HERO_ALT = "Mac Pro logic board on the repair bench at MacBook Repair Dubai, Media City";
const PARENT_HREF = "/mac-pro-repair-dubai";

const SERVICES: { Icon: LucideIcon; title: string; body: string; href: string }[] = [
  { Icon: Zap,         title: "GPU Repair",           href: "/mac-pro-gpu-repair-dubai",        body: "GPU corruption, display artefacts, or crashes under compute load? We repair Intel MPX module faults and M2 Ultra GPU subsystem issues." },
  { Icon: Monitor,     title: "No Display Fix",        href: "/mac-pro-no-display-dubai",        body: "Black screen on all ports at startup? We diagnose PCIe GPU slot faults (Intel) and M2 Ultra display port failures." },
  { Icon: Cpu,         title: "Logic Board Repair",    href: "/mac-pro-repair-dubai",            body: "Power rail faults, dead machine, or no POST? Component-level repair restores function without a full board replacement." },
  { Icon: Database,    title: "Data Recovery",         href: "/mac-data-recovery-dubai",         body: "Lost files after a board failure or failed SSD? We recover data from Mac Pro internal SSDs and PCIe storage cards." },
  { Icon: HardDrive,   title: "SSD Upgrade",           href: "/mac-pro-repair-dubai",            body: "More storage or faster NVMe? We upgrade Mac Pro SSDs on both Intel and M2 Ultra models using compatible expansion cards or direct upgrades." },
  { Icon: Power,       title: "Won't Turn On",         href: "/mac-pro-not-turning-on-dubai",    body: "No chime, no fan, no LED? We diagnose power supply faults, PCIe slot issues, and logic board failures on both Mac Pro generations." },
  { Icon: Thermometer, title: "Overheating Fix",       href: "/mac-pro-repair-dubai",            body: "Fan screaming under heavy workloads or unexpected shutdowns? We clean blower fans, replace thermal compound, and resolve board-level heat faults." },
  { Icon: Wind,        title: "Fan Cleaning",          href: "/mac-pro-repair-dubai",            body: "Dubai dust accumulates in Mac Pro blower fans. A professional clean restores airflow and reduces temperatures by 10–15 °C under sustained load." },
  ...
  { Icon: Search,      title: "Full Diagnostic",       href: "/mac-full-diagnostic-dubai",       body: "Not sure what is wrong? Our 30-minute diagnostic tests every subsystem — PCIe slots, GPU, storage, thermals — and gives you a written report." },
  { Icon: Droplets,    title: "Water Damage Repair",   href: "/macbook-water-damage-repair-dubai", body: "Liquid near a Mac Pro vent can reach the board fast. We open, dry, and clean at component level to stop corrosion spreading." },
  { Icon: Gauge,       title: "Running Slow",          href: "/mac-performance-tune-dubai",      body: "Slower than expected under professional workloads? We diagnose thermal throttling, GPU bottlenecks, and SSD performance issues." },
];
...
const PROCESS = [
  ...
  { Icon: Wrench,      title: "Expert repair",     body: "Your Mac Pro is repaired by a specialist with 21 years on Apple hardware, including Intel PCIe architecture and M2 Ultra logic board repair." },
  { Icon: ShieldCheck, title: "Quality check",     body: "Every repair passes a full function test: GPU output, PCIe slot integrity, storage speed, thermals — all verified before handover." },
];

const SIGNS: { Icon: LucideIcon; label: string; body: string }[] = [
  { Icon: Zap,          label: "GPU / display issues", body: "Display corruption, artefacts, or crashes under GPU compute — often a PCIe slot fault (Intel) or GPU subsystem issue (M2 Ultra)." },
  { Icon: Monitor,      label: "No display output",   body: "Black screen on all ports at startup. Could be a GPU module failure, PCIe power issue, or display port fault." },
  { Icon: Power,        label: "Won't turn on",        body: "No chime, no fan spin, no LED. Could be the power supply, a blown PCIe slot, or a logic board fault." },
  { Icon: Thermometer,  label: "Overheating",          body: "Blower fan running at full speed under load, or Mac Pro throttling during heavy GPU or CPU tasks." },
  { Icon: HardDrive,    label: "Storage errors",       body: "Disk Utility showing errors, files disappearing, or macOS refusing to boot from the internal SSD." },
  { Icon: AlertTriangle, label: "Frequent crashes",    body: "Kernel panics or random restarts during heavy workloads — GPU, logic board, or PCIe power fault." },
  { Icon: Droplets,     label: "Liquid damage",        body: "Any liquid inside the Mac Pro tower needs urgent attention. Power off and bring it in immediately." },
  { Icon: Gauge,        label: "Running slowly",       body: "Much slower than expected under professional workloads — thermal throttling, SSD degradation, or GPU bottleneck." },
];

const WHY: { Icon: LucideIcon; title: string; body: string }[] = [
  { Icon: Clock,       title: "Same-day service",         body: "Most Mac Pro repairs — GPU, overheating, no-display — are diagnosed and resolved within a few hours." },
  { Icon: Award,       title: "21 years experience",      body: "Our technicians have repaired Apple hardware since 2004, including Intel Mac Pro PCIe architecture and M2 Ultra board-level work." },
  ...
  { Icon: Truck,       title: "Free pickup Dubai-wide",   body: "Cannot bring it in? We collect your Mac Pro for free across Dubai and return it repaired." },
  ...
  { Icon: Search,      title: "Free diagnosis",           body: "Every Mac Pro gets a full diagnostic at no cost. We find the real fault before quoting." },
];

const TESTIMONIALS = [
  { name: "Khalid Al Suwaidi",  body: "Intel Mac Pro 2019 GPU MPX module failed during a client deadline. They sourced the part and replaced it the same day. Saved our production schedule." },
  { name: "Elena Volkova",      body: "M2 Ultra Mac Pro was showing display corruption on all ports. Board-level GPU fault found and fixed in one visit. Incredibly professional team." },
  { name: "David Okonkwo",      body: "Fan on our studio Mac Pro was screaming under rendering loads. After cleaning and thermal paste replacement it runs near-silent. Great service." },
];
...
export default function MacProGenerationPage(p: MacProGenerationProps) {
  useSeo(
    {
      title: p.seoTitle,
      ...
      <div className="-mb-[4rem] bg-bg text-text">
        ...
        <div className="pt-[64px]" style={{ borderBottom: "1px solid var(--color-border)" }}>
          <div className="mx-auto max-w-content px-5 md:px-6 py-[6px]">
            <BreadcrumbTrail tone="dark" trail={[
              ...
              { name: "Mac Pro Repair Dubai", path: PARENT_HREF },
              { name: `Mac Pro ${p.chip} Repair`, path: p.path },
            ]} />
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-[280px_1fr] gap-10 items-start">
            <div className="lg:sticky lg:top-24 rounded-xl border border-border/70 bg-bg-card ring-1 ring-black/[0.03] p-6">
              ...
              <p className="text-[14px] text-text-muted leading-relaxed">
                Everything you need to know before booking your Mac Pro {p.chip} repair.
              </p>
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[80px]">
          ...
          <div className="flex flex-wrap justify-center gap-sm text-[15px]">
            {[
              { label: "Mac Pro Repair Dubai",         href: PARENT_HREF },
              { label: "Mac Repair Dubai",             href: "/mac-repair-dubai" },
              { label: "Mac GPU Repair Dubai",         href: "/mac-pro-gpu-repair-dubai" },
        ...
        <section className="mt-[80px]" style={{ background: "#2C3137" }}>
          <div className="mx-auto max-w-content px-5 md:px-6 py-[48px] flex flex-col sm:flex-row items-center justify-between gap-md">
            <div>
              <p className="text-[11px] uppercase tracking-widest text-on-primary-muted mb-1">Ready to fix your Mac Pro?</p>
