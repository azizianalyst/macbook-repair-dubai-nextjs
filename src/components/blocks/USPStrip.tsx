...
export function USPStrip({ items = USPS, tone = "light" }: { items?: typeof USPS; tone?: "light" | "dark" }) {
  ...
  return (
    <div className={cn("border-y", dark ? "border border-white/10 bg-white/[0.04]" : "bg-bg-alt border-border")}>
      <ul className="mx-auto max-w-content px-5 md:px-6 py-md grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-md">
        {items.map((u) => {
          ...
          return (
            <li key={u.label} className={cn("flex items-center gap-sm text-[14px] font-medium", dark ? "text-on-primary" : "text-text")}>
              <Icon size={18} className={cn("shrink-0", dark ? "text-accent-bright" : "text-accent")} aria-hidden />
              <span>{u.label}</span>
            </li>
          );
        })}
