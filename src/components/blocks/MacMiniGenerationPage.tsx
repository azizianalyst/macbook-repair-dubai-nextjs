"use client";
// Shared layout for Mac mini generation hub pages (M1, M2, M4).
// Desktop-specific: no screen, no battery, no keyboard — SSD, display-out, logic board focus.
import {
  Star, MessageCircle, Phone,
  Monitor, HardDrive, Cpu, Thermometer,
  Power, Gauge, AlertTriangle, Wind, Wifi,
  Database, ShieldCheck, Clock, Award, Wrench,
...
export type MacMiniGenModel = { label: string; href?: string; aNum?: string };
export type MacMiniGenFAQ = { q: string; a: string };

export type MacMiniGenerationProps = {
  path: string;
  chip: string;        // "M4", "M2", "M1"
  years: string;       // "2024 – present"
  seoTitle: string;
  ...
  heroMeta: string[];
  models: MacMiniGenModel[];
  faqs: MacMiniGenFAQ[];
};

const HERO_IMG = "/images/real/lab/macbook-battery-internals-dubai.jpg";
const HERO_ALT = "Mac mini logic board on the repair bench at MacBook Repair Dubai, Media City";
const PARENT_HREF = "/mac-mini-repair-dubai";

const SERVICES: { Icon: LucideIcon; title: string; body: string; href: string }[] = [
  { Icon: Monitor,    title: "No Display Fix",       href: "/mac-mini-no-display-dubai",      body: "Black screen on boot, HDMI or Thunderbolt port not outputting? We diagnose display port faults, cable issues, and GPU logic on all Mac mini models." },
  { Icon: HardDrive,  title: "SSD Upgrade",          href: "/mac-mini-ssd-upgrade-dubai",     body: "Running out of storage? We upgrade M1 and M2 Mac mini SSDs. M4 models use soldered storage — we advise on the best path for your situation." },
  { Icon: Cpu,        title: "Logic Board Repair",   href: "/mac-mini-repair-dubai",          body: "Power rail faults, dead machine, or no video from any port? Component-level repair restores function without a full board replacement." },
  { Icon: Database,   title: "Data Recovery",        href: "/mac-data-recovery-dubai",        body: "Lost files after a failed SSD or board fault? We recover data from Mac mini SSDs including soldered M-chip storage." },
  { Icon: Power,      title: "Won't Turn On",        href: "/mac-mini-not-turning-on-dubai",  body: "Dead Mac mini? We diagnose power supply faults, SMC issues, and logic board failures to bring it back to life." },
  { Icon: Thermometer, title: "Overheating Fix",     href: "/mac-mini-overheating-fix-dubai", body: "Fan running at full speed or Mac mini throttling under load? We clean fans, replace thermal compound, and inspect for board-level heat faults." },
  { Icon: Wind,       title: "Fan Cleaning",         href: "/mac-mini-repair-dubai",          body: "Dubai dust builds up inside Mac mini fans fast. A professional fan clean restores airflow and drops idle temperatures by 10–15 °C." },
  { Icon: RefreshCw,  title: "OS Reinstall",         href: "/mac-os-reinstall-dubai",         body: "Corrupted macOS, bootloop, or startup failures? We reinstall macOS cleanly and restore your data from Time Machine or a manual backup." },
  { Icon: Search,     title: "Full Diagnostic",      href: "/mac-full-diagnostic-dubai",      body: "Not sure what is wrong? Our 30-minute diagnostic tests every subsystem — storage, memory, display out, thermals — and gives you a written report." },
  { Icon: Droplets,   title: "Water Damage Repair",  href: "/macbook-water-damage-repair-dubai", body: "Liquid reached your Mac mini? We open, dry, and clean the board at component level to stop corrosion before it spreads." },
  { Icon: Gauge,      title: "Running Slow",         href: "/mac-performance-tune-dubai",     body: "Slow startup or laggy apps? We tune macOS, clear login bloat, upgrade storage where possible, and replace aged thermal compound." },
  { Icon: Wifi,       title: "WiFi / Bluetooth",     href: "/mac-mini-repair-dubai",          body: "WiFi dropping or Bluetooth refusing to pair? We diagnose the antenna, AirPort card, and board-level Wi-Fi faults on all Mac mini models." },
];
...
const PROCESS = [
  ...
  { Icon: Wrench,      title: "Expert repair",     body: "Your Mac mini is repaired by a specialist with 21 years on Apple hardware, using OEM-grade parts and board-level tools." },
  { Icon: ShieldCheck, title: "Quality check",     body: "Every repair passes a full function test: display output on all ports, storage speed, thermals, and network — verified before handover." },
];

const SIGNS: { Icon: LucideIcon; label: string; body: string }[] = [
  { Icon: Monitor,     label: "No display output",   body: "Black screen on all ports at startup — HDMI and Thunderbolt both dead. Usually a GPU fault or display port issue." },
  { Icon: Power,       label: "Won't turn on",        body: "No chime, no fan spin, no LED. Could be a power supply fault, dead logic board, or failed SMC." },
  { Icon: Thermometer, label: "Overheating",          body: "Fan running constantly at full speed, or Mac mini shutting down under load. Dubai heat accelerates thermal issues." },
  { Icon: Gauge,       label: "Running slowly",       body: "Startup takes minutes, apps beach-ball constantly. Often a degraded SSD or thermal throttling from dust buildup." },
  { Icon: HardDrive,   label: "Storage errors",       body: "Disk Utility showing errors, files disappearing, or macOS refusing to mount the internal SSD." },
  { Icon: AlertTriangle, label: "Frequent crashes",   body: "Kernel panics or random restarts — usually a memory fault, logic board issue, or failing SSD." },
  { Icon: Droplets,    label: "Liquid damage",        body: "Any liquid near the Mac mini vent can reach the board. Power off immediately and bring it in." },
  { Icon: Wifi,        label: "WiFi / Bluetooth",     body: "Connections drop after sleep or Bluetooth refuses to pair — antenna or AirPort module fault." },
];

const WHY: { Icon: LucideIcon; title: string; body: string }[] = [
  { Icon: Clock,       title: "Same-day service",         body: "Most Mac mini repairs — logic board, overheating, no-display — are diagnosed and fixed within a few hours." },
  { Icon: Award,       title: "21 years experience",      body: "Our technicians have repaired Apple hardware since 2004, across every Mac mini generation from G4 to M4." },
  { Icon: ShieldCheck, title: "Warranty up to 12 months", body: "All parts and labour carry a written warranty. Logic board and SSD repairs come with 12-month cover." },
  { Icon: Truck,       title: "Free pickup Dubai-wide",   body: "Cannot bring it in? We collect your Mac mini for free across Dubai and return it repaired." },
  ...
  { Icon: Search,      title: "Free diagnosis",           body: "Every Mac mini gets a full diagnostic at no cost. We find the real fault before quoting." },
];

const TESTIMONIALS = [
  { name: "Fahad Al Mansoori", body: "My M4 Mac mini stopped outputting video entirely. They diagnosed a Thunderbolt port fault and had it fixed the same day. Excellent service." },
  { name: "Priya Nambiar",     body: "SSD upgrade on my M2 Mac mini done in under 2 hours. The technician cloned my drive perfectly — not a single file missing. Highly recommend." },
  { name: "Marcus Weber",      body: "Fan was screaming at full speed in our office Mac mini. They cleaned it, replaced thermal paste and it now runs silently. Quick and professional." },
];
...
export default function MacMiniGenerationPage(p: MacMiniGenerationProps) {
  useSeo(
    {
      title: p.seoTitle,
      ...
      <div className="-mb-[4rem] bg-bg text-text">
        ...
        <div className="pt-[64px]" style={{ borderBottom: "1px solid var(--color-border)" }}>
          <div className="mx-auto max-w-content px-5 md:px-6 py-[6px]">
            <BreadcrumbTrail tone="dark" trail={[
              ...
              { name: "Mac mini Repair Dubai", path: PARENT_HREF },
              { name: `Mac mini ${p.chip} Repair`, path: p.path },
            ]} />
          </div>
        </div>
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[80px]">
          <SectionHeading
            ...
            title={`Which Mac mini ${p.chip} models do we repair?`}
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[96px]">
          <SectionHeading
            label="What we fix"
            title={`Mac mini ${p.chip} repair services`}
            sub="From a no-display fault to a dead logic board, our technicians handle every Mac mini repair in our Dubai Media City workshop."
          />
        ...
        <section className="bg-bg-alt mt-[96px] py-[80px]" style={{ borderTop: "1px solid var(--color-border)", borderBottom: "1px solid var(--color-border)" }}>
          <div className="mx-auto max-w-content px-5 md:px-6">
            <SectionHeading
              label="How it works"
              title="How does the Mac mini repair process work?"
              sub="Simple, transparent, and fast. Here is what happens when you bring in your Mac mini."
            />
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[96px]">
          <SectionHeading
            label="Warning signs"
            title={`What are the signs your Mac mini ${p.chip} needs repair?`}
          />
          ...
          <div className="grid lg:grid-cols-[280px_1fr] gap-10 items-start">
            <div className="lg:sticky lg:top-24 rounded-xl border border-border/70 bg-bg-card ring-1 ring-black/[0.03] p-6">
              ...
              <p className="text-[14px] text-text-muted leading-relaxed">
                Everything you need to know before booking your Mac mini {p.chip} repair.
              </p>
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[80px]">
          ...
          <div className="flex flex-wrap justify-center gap-sm text-[15px]">
            {[
              { label: "Mac mini Repair Dubai",         href: PARENT_HREF },
              { label: "Mac Repair Dubai",              href: "/mac-repair-dubai" },
              { label: "Mac SSD Upgrade Dubai",         href: "/mac-ssd-upgrade-dubai" },
        ...
        <section className="mt-[80px]" style={{ background: "#2C3137" }}>
          <div className="mx-auto max-w-content px-5 md:px-6 py-[48px] flex flex-col sm:flex-row items-center justify-between gap-md">
            <div>
              <p className="text-[11px] uppercase tracking-widest text-on-primary-muted mb-1">Ready to fix your Mac mini?</p>
