      ...
      {steps.map((s, i) => (
        <Reveal
          ...
          className={cn(
            ...
            dark
              ? "border-white/10 bg-white/[0.04] hover:border-border-strong"
          ...
          <span
            ...
            className={cn(
              ...
              dark
                ...
                : "bg-primary text-on-primary shadow-sm ring-bg-alt",
            )}
          >
            {i + 1}
          </span>
          <div>
            <p className={cn("font-semibold text-[16px] mb-1", dark && "text-on-primary")}>{s.title}</p>
            <p className={cn("text-[14px] leading-relaxed", dark ? "text-on-primary-muted" : "text-text-muted")}>{s.body}</p>
          </div>
        </Reveal>
      ))}
    </ol>
