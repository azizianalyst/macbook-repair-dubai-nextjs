"use client";
// Shared layout for MacBook Air generation hub pages (M1, M2, M3, M4, M5, Intel).
// Air-specific: fanless design, no Touch Bar, no ProMotion, no mini-LED, no OLED.
import {
  Star, MessageCircle, Phone,
  Keyboard, Monitor, BatteryCharging, Droplets, PlugZap,
  MousePointer2, HardDrive, Cpu, Thermometer,
  Power, Gauge, AlertTriangle, Flame, Wifi,
  Link2, ShieldCheck, Clock, Award, Wrench,
  Search, BadgeCheck, Truck,
} from "lucide-react";
...
export type AirGenModel = { label: string; href?: string; aNum?: string };
export type AirGenFAQ = { q: string; a: string };

export type MacBookAirGenerationProps = {
  path: string;
...
const HERO_ALT = "MacBook Air open on the repair bench at MacBook Repair Dubai, Media City";
const PARENT_HREF = "/macbook-air-repair-dubai";
...
const SERVICES: { Icon: LucideIcon; title: string; body: string; href: string }[] = [
  ...
  { Icon: MousePointer2,   title: "Trackpad Repair",      href: "/macbook-trackpad-repair-dubai",      body: "Trackpad not clicking or behaving erratically? We recalibrate, repair, or replace Force Touch trackpads on all Air models." },
  ...
  { Icon: Thermometer,     title: "Overheating Fix",      href: "/macbook-overheating-fix-dubai",      body: "MacBook Air is fanless. Dubai heat plus sustained load causes throttling. We reclean thermal pads and inspect for board faults." },
  ...
  { Icon: Gauge,           title: "Running Slow",         href: "/mac-performance-tune-dubai",         body: "Slow startup or sluggish apps? We tune macOS, clear startup bloat, and replace degraded thermal pads." },
...
const SIGNS: { Icon: LucideIcon; label: string; body: string }[] = [
  { Icon: Gauge,           label: "Running slowly",      body: "Long startup, lag on basic tasks, or thermal throttle from Dubai heat with no fan." },
  { Icon: Thermometer,     label: "Overheating",         body: "Chassis too hot to touch at idle. Fanless design means heat exits through aluminium." },
  { Icon: BatteryCharging, label: "Battery problems",    body: "Drains in 2-3 hours instead of 15+, won't charge, or the case is bulging." },
  { Icon: Monitor,         label: "Screen issues",       body: "Cracked Liquid Retina, flickering, dead backlight, or colour lines." },
  { Icon: Keyboard,        label: "Keyboard / trackpad", body: "Keys stuck, double-typing, butterfly failure, or trackpad won't click." },
  { Icon: Droplets,        label: "Liquid damage",       body: "Spilled liquid — power off immediately and bring it in for board cleaning." },
  { Icon: AlertTriangle,   label: "Frequent crashes",    body: "Random shutdowns, kernel panics, or black screens on wake." },
  { Icon: Wifi,            label: "WiFi / Bluetooth",    body: "WiFi dropping after sleep or Bluetooth refusing to pair with any device." },
  { Icon: Flame,           label: "Burning smell",       body: "Any burning or unusual smell needs urgent attention — bring it in same day." },
];

const WHY: { Icon: LucideIcon; title: string; body: string }[] = [
  { Icon: Clock,       title: "Same-day service",          body: "Most MacBook Air repairs — screen, battery, keyboard — are done within a few hours, not days." },
  { Icon: Award,       title: "21 years experience",       body: "Our technicians have repaired Apple hardware since 2004. We have seen every Air fault, across every chip generation." },
  ...
  { Icon: Truck,       title: "Free pickup Dubai-wide",    body: "Cannot come to us? We collect your MacBook Air for free across Dubai and return it repaired." },
  ...
  { Icon: Search,      title: "Free diagnosis",            body: "Every MacBook Air gets a full diagnosis at no cost. We find the real fault, not guess at it." },
];

const TESTIMONIALS = [
  { name: "Saeed Alam",           body: "Dropped my MacBook Air and the screen completely shattered. They replaced it same day for a fair price and it looks brand new. Highly recommended." },
...
export default function MacBookAirGenerationPage(p: MacBookAirGenerationProps) {
  useSeo(
    {
      title: p.seoTitle,
      ...
      <div className="-mb-[4rem] bg-bg text-text">
        ...
        <div className="pt-[64px]" style={{ borderBottom: "1px solid var(--color-border)" }}>
          <div className="mx-auto max-w-content px-5 md:px-6 py-[6px]">
            <BreadcrumbTrail tone="dark" trail={[
              ...
              { name: "MacBook Air Repair Dubai", path: PARENT_HREF },
              { name: `MacBook Air ${p.chip} Repair`, path: p.path },
            ]} />
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-[280px_1fr] gap-10 items-start">
            <div className="lg:sticky lg:top-24 rounded-xl border border-border/70 bg-bg-card ring-1 ring-black/[0.03] p-6">
              ...
              <p className="text-[14px] text-text-muted leading-relaxed">
                Everything you need to know before booking your MacBook Air {p.chip} repair.
              </p>
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[80px]">
          ...
          <div className="flex flex-wrap justify-center gap-sm text-[15px]">
            {[
              { label: "MacBook Air Repair Dubai", href: PARENT_HREF },
