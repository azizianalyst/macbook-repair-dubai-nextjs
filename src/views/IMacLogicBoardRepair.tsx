...
export default function IMacLogicBoardRepair() {
  return (
    <DevicePageTemplate
      family="imac"
      seoTitle="iMac Logic Board Repair Dubai | Component-Level | Call 055 741 3706"
      seoDescription={`iMac logic board repair Dubai. Component-level rework on 27" Intel and 24" M-series. Power delivery, GPU, T2 chip, SoC limits explained. Message us for a quote.`}
      path="/imac-logic-board-repair-dubai"
      eyebrow="iMac logic board repair"
      h1="iMac Logic Board Repair Dubai"
      subtitle="Component-level rework where it's possible, board swap where it isn't. Power delivery, GPU failure, T2 chip lockup. Message us on WhatsApp for a price on your iMac."
      startingPrice={1000}
      timeline="3-5 days"
      ...
      intro={
        <>
          <p className="text-[18px] leading-relaxed mb-md">
            iMac logic boards fail in three patterns: <strong>power delivery</strong> (no boot, fans only), <strong>GPU failure</strong> (boots to a black or distorted screen), and <strong>T2 or SoC lockup</strong> (chime, backlight, but no display handoff). Each has a different fix, and we'll quote your exact fault on WhatsApp.
          </p>
          <p className="text-[16px] text-text-muted leading-relaxed mb-md">
            On 2017-2020 27" Intel iMacs we work at component level - replacing the failed PMIC, MOSFET, capacitor or buck converter on the board itself. That's a 3-5 day job and costs far less than a full board swap. GPU rework on the discrete AMD Radeon Pro chip is also viable in many cases.
          </p>
      ...
      problems={[
        { title: "No power, no chime, no fan", body: "PMIC or input-side fault. Common on 2017-2019 27\" iMacs after a power surge or PSU failure. Component-level rework - message us for the price." },
        { title: "Fan spins, no display", body: "Boot completes but display handoff fails - usually GPU on Intel 27\" iMacs (Radeon Pro 575/580). Reflow rarely lasts; we replace the GPU itself with reballed stock. Message us for a quote." },
        ...
        { title: "T2 chip lockup (Intel 2018+)", body: "iMac chimes, fans run, backlight comes on, then nothing - T2 secure subsystem fault. Specialised JTAG repair - message us for the price. Data preserved." },
        { title: "USB-C / Thunderbolt port dead", body: "PD controller or retimer fault. Component-level fix - price depends on which port and which controller chip; message us for a quote." },
      ...
      comparisonRows={[
        ["Starting price",          "Price on request",                   "Whole-board swap only"],
        ["Component-level option",  "Yes - saves the original board",     "No - board-swap only"],
        ["GPU rework (Intel)",      "Yes - with reballed chip",           "Not offered - board swap only"],
        ["Diagnostic fee",          "Free - refunded if you don't proceed", "Charged"],
      ...
      faqs={[
        { q: `Why is component-level repair cheaper than a board swap?`, a: `Because we replace the actual failed part - a single chip, the GPU, or the PMIC - instead of the entire logic board. The labour is high (microscope, hot-air station, JTAG) but it still works out at a fraction of board-swap pricing. Message us for your exact quote.` },
        ...
        { q: "Why does T2 chip work cost more?", a: "T2 repair needs JTAG hardware, a paired backup of the secure enclave, and a much slower diagnostic process. Done wrong, the SSD becomes unreadable forever. The price reflects the time and the equipment, not arbitrary mark-up - message us for a quote." },
        ...
        { q: "Should I just buy a new iMac instead?", a: "Sometimes yes. If your 2017 iMac needs major board work plus a new screen, we'll tell you straight - a refurbished M3 24\" iMac is a better buy. Honest advice is the foundation of the workshop. Message us and we'll help you weigh it up." },
      ]}
