...
export default function IPhoneFaceIdRepair() {
  return (
    <IPhonePageTemplate
      seoTitle="iPhone Face ID Repair Dubai - Honest Success Rates | No-Fix-No-Charge"
      seoDescription="iPhone Face ID repair Dubai with honest success rates. Sensor work where calibration allows. Apple can't repair Face ID - we sometimes can. Message us for a quote."
      ...
      honestyCallout={
        <div className="text-[15px] text-text leading-relaxed space-y-md">
          ...
          <p>
            <strong>Honest alternatives:</strong> if Face ID can't be restored, iOS still works - just with passcode entry and Touch-ID-style biometric absent. Many customers live with this rather than pay for a costly Apple service-unit swap. The workshop's job is to help you decide.
          </p>
        </div>
      }
      problems={[
        { title: "Face ID stopped working after a screen swap (done elsewhere)", body: "Sensor stack wasn't transferred from the original screen. Workshop opens the phone, retrieves the original sensors, transfers correctly. Same day - message us for the price. 95% success when the original sensors are intact." },
        { title: "'Face ID is not available, try setting up later'", body: "iOS error indicating one of the TrueDepth sensors isn't responding. Free diagnosis identifies which one - sometimes a dirty notch (free clean), sometimes the flood illuminator, sometimes the IR camera. Message us for the exact price once we know which." },
        ...
        { title: "Move iPhone Lower / Higher message - never recognises face", body: "Geometry of the dot projector pattern may be off due to lens damage or alignment shift. Calibration sometimes possible - message us for the price; replacement rarely successful." },
        ...
        { title: "Face ID fails only in dark or bright sunlight", body: "Flood illuminator (dark) or IR camera (bright) failing. Component-level replacement - message us on WhatsApp for the exact price on your model." },
        ...
        { title: "iPhone X (oldest Face ID phone) - Face ID died of age", body: "After 5-7 years, dot projector lasers degrade. Replacement on iPhone X has the highest success rate of any Face ID repair (~70%) because pairing was looser in the first generation. Around 3 days - message us for the price." },
      ]}
      steps={[
        ...
        { title: "Test in Settings > Face ID & Passcode", body: "Customer attempts to set up Face ID. If successful: charge as quoted. If unsuccessful: phone returned with no charge for the attempted repair (only a small diagnostic fee if the original component was replaced unsuccessfully)." },
      ...
      warrantyDays={365}
      warrantyBullets={[
        "No-fix-no-charge: if Face ID isn't restored after the repair attempt, only a small diagnostic fee is charged.",
      ...
      comparisonRows={[
        ["Apple's official approach",   "Whole-device service swap (does not repair Face ID)", "Costly service-unit swap"],
        ["Workshop's approach",         "Component-level repair attempt with no-fix-no-charge", "Service unit (full price)"],
        ["Sensor stack transfer (screen repair)", "95% success - price on request",            "Yes - included in genuine repair"],
        ["Flood illuminator replacement","~70% success - price on request",                    "Not offered"],
        ["IR camera replacement",       "~50% success - price on request",                     "Not offered"],
        ["Dot projector replacement",   "~30% success (honest) - price on request",            "Not offered"],
      ...
      faqs={[
        ...
        { q: "Why won't Apple repair Face ID?", a: "Apple's design ties the TrueDepth components to the Secure Enclave through factory pairing that hasn't been made available outside Apple. Apple's official policy is whole-device replacement when Face ID fails - a costly service-unit swap. The workshop's component-level approach is the only repair-and-keep-your-phone alternative in Dubai." },
        { q: "How much does Face ID repair cost?", a: "It depends entirely on which component failed - sensor stack transfer, flood illuminator, IR camera, dot projector, or a full TrueDepth assembly transfer. Diagnosis is free, and no-fix-no-charge applies: if the repair attempt doesn't restore Face ID, only a small diagnostic fee is charged. Message us on WhatsApp with your model for today's exact price." },
        { q: "Can I just live without Face ID?", a: "Yes - many customers choose to. iOS works with passcode entry instead. Apple Pay, app sign-ins, and password autofill all fall back to passcode. The phone is fully functional, just with the extra step of typing the passcode. Many customers prefer this to paying for a full Apple service unit." },
        { q: "My screen was replaced and now Face ID doesn't work - can you fix it?", a: "Yes - most likely the previous repair didn't transfer the sensor stack from the original screen. The workshop opens the phone, retrieves the original sensors, transfers them correctly. Same day, 95% success rate - message us for the price. This is the most reliable Face ID repair we offer." },
