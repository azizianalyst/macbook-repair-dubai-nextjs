...
const HERO_FEATURES = [
  { icon: Tag,    label: "Price on request" },
...
const PRICE_TABLE = [
  { r: "Screen Replacement", c: "Price on request", n: "Air 13\" / Pro 13\" / Pro 14\" / Pro 16\" - message us for your model's exact price." },
  { r: "Battery Replacement", c: "Price on request", n: "Air and Pro 13\"/14\"/16\", with a 3-month warranty. Message us for the exact price." },
  { r: "Keyboard Replacement", c: "Price on request", n: "Single key, Magic Keyboard swap or butterfly top case - message us for the exact price." },
  { r: "Logic Board Repair", c: "Price on request", n: "Component-level repair; depends on the fault and model." },
  { r: "Hard Drive Replacement (HDD to SSD)", c: "Price on request", n: "Includes SSD cost and installation." },
  { r: "Water Damage Repair", c: "Price on request", n: "Depends on the extent of damage. No fix, no charge." },
  { r: "Software Issues (OS Install, Virus Removal)", c: "Price on request", n: "Simple fixes are usually less expensive." },
];
...
const FAQS = [
  { q: "How much does MacBook repair cost in Dubai?", a: "MacBook repair at MacBook Repair Dubai is priced by the fault and the model, so screen, battery, keyboard, logic board and water-damage jobs each have their own rate. Diagnosis is always free and the exact price is confirmed in writing on WhatsApp before any work begins - message us your model for today's price. All prices are VAT-inclusive." },
  { q: "How much is a MacBook screen replacement in Dubai?", a: "MacBook screen replacement at MacBook Repair Dubai is priced by model, so the Air 13\", the Pro 13\", the 14\" Pro and the 16\" Pro each have their own rate. Flexgate backlight failure on the 2016 to 2017 Pro is often just a cable swap rather than a full panel. Message us your model on WhatsApp for the exact price. Most screens are done the same day, and True Tone stays intact when you choose the genuine Apple panel at quote time." },
  { q: "How much does it cost to replace a MacBook battery in Dubai?", a: "A MacBook battery replacement is priced by model, covering the Air 11\", 13\", and 15\" and the Pro 13\", 14\" and 16\" on both Intel and Apple Silicon. The price covers a tier-1 cell, the labour, the firmware re-pair on Apple Silicon, and the calibration cycle, with a 3-month warranty and an 80% capacity guarantee. Message us your model on WhatsApp for the exact price. The job takes about 2 hours on the bench at the Concord Tower workshop in Dubai Media City." },
  { q: "Is it worth repairing an old MacBook or should I replace it?", a: "It is usually worth repairing if the fix costs less than half the price of a comparable replacement. A battery or screen swap on a MacBook that still holds strong resale value is an easy decision, and component-level logic board repair beats a full Apple board swap. MacBook Repair Dubai gives an honest opinion after the free diagnosis, and if the board is unrecoverable you pay nothing under the no-fix-no-charge policy - message us on WhatsApp for your model's exact price." },
  ...
  { q: "Will I lose my data when you repair my MacBook?", a: "Your data stays on the SSD for standard repairs like screen, battery, and keyboard, since the storage is never touched. For logic board and water damage work the data is preserved wherever the board survives. If the board is dead, separate data recovery is available - message us on WhatsApp for the price - with strong odds on Intel logic boards and lower odds on T2 and Apple Silicon models, where the SSD is encrypted to the secure enclave and recovery depends on the chip surviving. MacBook Repair Dubai gives every customer a signed data-privacy guarantee on intake: your data is never accessed, copied, or shared for any reason." },
  { q: "Can you fix a water-damaged MacBook in Dubai?", a: "MacBook Repair Dubai recovers liquid-damaged MacBooks with full ultrasonic cleaning and corrosion removal. The workshop logbook shows a 90% recovery rate when the MacBook arrives within 24 hours and was not powered on after the spill, dropping to about 60% at 72 hours. Power it off, do not charge it, skip the rice, and WhatsApp 055 741 3706 for same-hour free emergency pickup across Dubai mainland. No fix, no charge if the board is beyond saving." },
  { q: "My MacBook won't turn on, can you fix it?", a: "A MacBook that won't power on is most often a dead battery, a failed DC-in charging board, or a logic board fault, and MacBook Repair Dubai diagnoses all three for free. Whether it needs a battery, a charging-board swap or component-level board repair, the fault is confirmed before any charge, so you know the price up front - message us on WhatsApp for the exact figure. Bring it in or book free pickup across Dubai mainland." },
  { q: "What should I do about a swollen MacBook battery?", a: "Stop using the MacBook immediately and do not charge it or try to power it on, because a swollen lithium pack is a fire risk and can crack the trackpad. MacBook Repair Dubai replaces swollen batteries the same day in about 2 hours, with a 3-month warranty. WhatsApp 055 741 3706 for same-hour free emergency pickup across Dubai mainland and today's exact price. If the swelling has already cracked the trackpad, we can do the combined battery and trackpad job in one visit." },
  { q: "How much does it cost to fix a MacBook keyboard in Dubai?", a: "MacBook keyboard repair ranges from a single key cap on the Magic Keyboard to a full Magic Keyboard swap on Apple Silicon. The 2016 to 2019 butterfly top case costs more, because Apple bonded the keyboard, battery, and speakers into one unit - but we still come in well under Apple's own list price. Arabic, Russian, or French layout swaps are also available. Message us your model on WhatsApp for the exact price. Most are same day at MacBook Repair Dubai." },
  ...
  { q: "How does your pricing compare to the Apple Store?", a: "MacBook Repair Dubai is consistently below Apple's UAE list prices, with no upfront service fee. A Magic Keyboard swap, a butterfly top case, and component-level board repair all come in well under Apple's equivalent charge - and a board-level repair replaces a full Apple board swap that costs far more. Diagnosis is free. Message us your model on WhatsApp for the exact price. Same outcome, a fraction of the cost." },
    ...
    <PageShell>
      <div className="bg-bg-alt text-text -mb-[4rem]">
          ...
          <div className="relative mx-auto max-w-content px-5 md:px-6 grid gap-2xl md:grid-cols-12 items-start">
            <div className="md:col-span-6">
              ...
              <div className="mt-md flex flex-wrap gap-2 text-[13px] font-medium">
                <span className="inline-flex items-center rounded-full border border-border/70 bg-bg-card ring-1 ring-black/[0.03] px-3 py-1 text-text">Screen repair &mdash;&nbsp;<span className="font-semibold text-accent">price on request</span></span>
        ...
        <section id="pricing" className="scroll-mt-24 mx-auto max-w-content px-5 md:px-6 py-4xl">
          <SectionHead eyebrow="Transparent pricing" title="How Much Does a MacBook Repair Cost?" intro="Your repair price varies with the type of repair, the MacBook model, any additional repairs, and current discounts. Message us on WhatsApp for today's exact price on your model - here's a general list of common repairs." />
          <ul className="mb-xl grid gap-2 sm:grid-cols-2 lg:grid-cols-4 list-none p-0 m-0 text-[15px] text-text-muted">
            <li className="flex items-start gap-2"><Wallet size={18} className="text-accent mt-0.5 shrink-0" aria-hidden /> <span><strong className="text-text">Price:</strong> On request via WhatsApp</span></li>
            ...
            <table className="w-full border-collapse text-left text-[14px] min-w-[560px]">
              <thead>
                <tr className="border-b border-border text-accent">
                  ...
                  <th className="px-lg py-md font-semibold whitespace-nowrap">Estimated Cost</th>
