"use client";
import SubServicePageTemplate from "@/components/blocks/SubServicePageTemplate";
import { Link } from "@/lib/router-compat";

export default function IPhone16ProRepair() {
  return (
    <SubServicePageTemplate
      seoTitle="iPhone 16 Pro Repair Dubai - 6.3-inch A18 Pro | 055 741 3706"
      seoDescription="iPhone 16 Pro (2024) repair Dubai. A18 Pro, 6.3-inch ProMotion, titanium, Camera Control, USB-C 3. Same-day, warranty up to 12 months. WhatsApp for today's price."
      path="/iphone-16-pro-repair-dubai"
      eyebrow="iPhone 16 Pro (2024, discontinued Sept 2025)"
      h1="iPhone 16 Pro Repair Dubai - Titanium, ProMotion, Camera Control"
      subtitle="The 6.3-inch Pro (larger from 6.1) - A18 Pro, titanium, ProMotion always-on, USB-C 3, Camera Control. Discontinued Sept 2025 but fully serviceable. Message us on WhatsApp for today's price."
      startingPrice={350}
      timeline="Same day"
      whatsappPrefill="Hi, I have an iPhone 16 Pro (2024) and need help with:"
      breadcrumb={[
        { name: "Home",          path: "/" },
        { name: "iPhone Repair", path: "/iphone-repair-dubai" },
        { name: "iPhone 16 Pro", path: "/iphone-16-pro-repair-dubai" },
      ]}
      technician={{ name: "Shafiq", years: 15, specialisation: "iPhone board-level repair" }}
      serviceName="iPhone 16 Pro Repair Dubai"
      intro={
        <>
          <p className="text-[18px] leading-relaxed mb-md">
            <strong>Quick answer:</strong> The iPhone 16 Pro (Sept 2024) - A18 Pro, 6.3-inch ProMotion LTPO OLED (grew from the 15 Pro's 6.1") with Dynamic Island and AOD, titanium frame, USB-C 3 (10 Gbps), Action Button + Camera Control, 48MP main + 5× tetraprism telephoto + 48MP ultra-wide + LiDAR. Discontinued in Sept 2025 with the 17 Pro launch but parts are abundant. We handle screen, Camera Control and telephoto repairs - message us on WhatsApp for today's exact price on your model.
          </p>
          <h2 className="text-[24px] md:text-[28px] mb-md mt-lg">About the iPhone 16 Pro</h2>
          <ul className="space-y-1 text-[15px] mb-lg">
            <li>• <strong>Released:</strong> September 2024 · <strong>discontinued</strong> September 2025</li>
            <li>• <strong>Chip:</strong> Apple A18 Pro (8 GB RAM)</li>
            <li>• <strong>Display:</strong> 6.3-inch ProMotion LTPO OLED, AOD, Dynamic Island, 2622 × 1206</li>
            <li>• <strong>Camera:</strong> 48 MP main + 12 MP 5× tetraprism telephoto + 48 MP ultra-wide + LiDAR</li>
            <li>• <strong>Connector:</strong> USB-C 3 (10 Gbps) · MagSafe + Qi · 5G sub-6 · IP68</li>
            <li>• <strong>Frame:</strong> Grade-5 titanium · Action Button · Camera Control · Apple Intelligence</li>
            <li>• <strong>iOS support:</strong> iOS 18 native · iOS 19-23 likely</li>
          </ul>
          <h2 className="text-[24px] md:text-[28px] mb-md">Common problems we see on the 16 Pro</h2>
          <ul className="space-y-2 text-[15px] mb-lg">
            <li>• <strong>Camera Control button issues</strong></li>
            <li>• <strong>5x tetraprism telephoto failure</strong></li>
            <li>• <strong>USB-C 3 port wear</strong></li>
            <li>• <strong>Cracked 6.3-inch ProMotion OLED</strong></li>
            <li>• <strong>Titanium frame dents</strong> (cosmetic)</li>
            <li>• <strong>Apple Intelligence heat</strong></li>
            <li>• <strong>Battery service</strong> (1-2 year-old units)</li>
            <li>• <strong>Face ID intermittent</strong></li>
          </ul>
          <h2 className="text-[24px] md:text-[28px] mb-md">Services available for the 16 Pro</h2>
          <div className="overflow-x-auto border border-border rounded-md bg-bg-card mb-lg">
            <table className="w-full text-[14px] min-w-[560px]">
              <thead className="bg-bg-card"><tr className="text-left"><th className="px-md py-sm font-semibold">Service</th><th className="px-md py-sm font-semibold">Price</th><th className="px-md py-sm font-semibold">Timeline</th><th className="px-md py-sm font-semibold">Book</th></tr></thead>
              <tbody>
                <tr><td className="px-md py-sm">6.3-inch ProMotion OLED replacement</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">Same day</td><td className="px-md py-sm"><Link to="/iphone-screen-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr className="bg-bg-alt"><td className="px-md py-sm"><Link to="/iphone-16-pro-battery-capacity" className="text-accent underline underline-offset-2">Battery replacement</Link> (3,582 mAh)</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">45 min</td><td className="px-md py-sm"><Link to="/iphone-battery-replacement-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr><td className="px-md py-sm">USB-C 3 port (Thunderbolt) replacement</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">Same day</td><td className="px-md py-sm"><Link to="/iphone-usb-c-port-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr className="bg-bg-alt"><td className="px-md py-sm">Camera Control button repair</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">Same day</td><td className="px-md py-sm"><Link to="/iphone-power-button-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr><td className="px-md py-sm">Action Button repair</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">Same day</td><td className="px-md py-sm"><Link to="/iphone-power-button-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr className="bg-bg-alt"><td className="px-md py-sm">5x tetraprism telephoto module</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">1-2 days</td><td className="px-md py-sm"><Link to="/iphone-camera-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr><td className="px-md py-sm">Full triple camera + LiDAR</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">Same day</td><td className="px-md py-sm"><Link to="/iphone-camera-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr className="bg-bg-alt"><td className="px-md py-sm">Face ID repair</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">1-3 days</td><td className="px-md py-sm"><Link to="/iphone-face-id-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
                <tr><td className="px-md py-sm">Titanium frame polish (cosmetic)</td><td className="px-md py-sm">Price on request</td><td className="px-md py-sm">1-2 days</td><td className="px-md py-sm"><Link to="/iphone-back-glass-repair-dubai" className="text-accent font-semibold hover:underline">Book →</Link></td></tr>
              </tbody>
            </table>
          </div>
          <h2 className="text-[24px] md:text-[28px] mb-md">Are parts still available in 2026?</h2>
          <p className="text-[15px] text-text-muted leading-relaxed mb-lg">
            6.3-inch ProMotion OLED with Dynamic Island in stock, including genuine Apple-pull panels. Tetraprism module and Camera Control flex are sealed assemblies. USB-C 3 in stock. Message us on WhatsApp for today's price on any part.
          </p>
          <h2 className="text-[24px] md:text-[28px] mb-md">Is the 16 Pro still worth repairing in 2026?</h2>
          <p className="text-[15px] leading-relaxed mb-lg">
            <strong>Strong yes.</strong> Only ~1.5 years old, A18 Pro, 8 GB RAM, and strong used resale value. iOS 23 expected. A repair costs far less than replacing the phone - message us for the exact price and decide with real numbers.
          </p>
        </>
      }
      whatWeFix={[
        "Camera Control button stuck / unresponsive",
        "5x tetraprism telephoto failure",
        "USB-C 3 / Thunderbolt port wear",
        "Cracked 6.3-inch ProMotion OLED",
        "Titanium frame dents (cosmetic polish)",
        "Battery service (1-2 years in)",
        "Apple Intelligence heat throttling",
        "Face ID intermittent / failed",
      ]}
      steps={[
        { title: "WhatsApp the fault",   body: "Photo or short clip. Quote in 4 minutes." },
        { title: "Free pickup",          body: "Across Dubai mainland." },
        { title: "Diagnose + repair",    body: "Standard same day; tetraprism + Face ID 1-3 days." },
        { title: "QC + return",          body: "ProMotion, AOD, Dynamic Island, USB-C 3, 5x zoom, Camera Control, Action Button and Face ID tested, signed warranty, free delivery." },
      ]}
      warrantyDays={365}
      warrantyBullets={[
        "12 months on parts and labour.",
        "Screen: dead pixels, ghost touch, 120Hz refresh, AOD covered.",
        "Camera Control: gestures + haptic verified.",
        "USB-C 3: full 10 Gbps data + 27W charge tested.",
      ]}
      faqs={[
        { q: "Why did Apple grow the Pro from 6.1\" to 6.3\"?", a: "Slimmer bezels and a slightly larger chassis allowed Apple to grow the display without growing the phone significantly. Repair-wise this means a different OLED part - not interchangeable with the 15 Pro." },
        { q: "Bundle pricing?", a: "Yes - bundling repairs like screen + battery or Camera Control + USB-C saves money versus booking them separately. Message us on WhatsApp for today's bundle price on your model." },
        { q: "Is the 16 Pro still supported by Apple?", a: "Yes for software (iOS 18 native, iOS 19+ guaranteed). Discontinued in Sept 2025 with the iPhone 17 Pro launch but parts and service remain available." },
        { q: "Genuine Apple OLED?", a: "Yes - we offer both genuine Apple-pull and OEM-grade OLED panels. Message us on WhatsApp for the current price on each option." },
        { q: "Same day?", a: "OLED, battery, USB-C, Camera Control, Action Button, main + ultra-wide cameras - yes." },
      ]}
      reviewNames={["Seyed Moulana", "B Parker"]}
      related={[
        { label: "iPhone Camera Repair Dubai",      href: "/iphone-camera-repair-dubai",      description: "5x tetraprism + 48MP ultra-wide module repair." },
        { label: "iPhone Screen Repair Dubai",      href: "/iphone-screen-repair-dubai",      description: "6.3-inch ProMotion OLED replacement." },
        { label: "iPhone USB-C Port Repair Dubai",  href: "/iphone-usb-c-port-repair-dubai",  description: "USB-C 3 / Thunderbolt port replacement." },
        { label: "iPhone 16 Pro battery capacity", href: "/iphone-16-pro-battery-capacity", description: "Specs and replacement details for the iPhone 16 Pro battery." },
      ]}
    />
  );
}
