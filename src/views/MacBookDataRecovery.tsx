...
const COMMON_SCENARIOS = [
  { title: "MacBook won't power on - files trapped on a working SSD",
    body: "Most common ticket. The logic board is dead but the SSD chips themselves are healthy. On Intel pre-T2 (2010-2017) the workshop pulls the SSD and reads it on a chip-off rig, with an 80% success rate. On T2 and Apple Silicon the SSD is encrypted and bonded to the secure enclave - board repair becomes the recovery path." },
  { title: "Accidentally deleted important files (emptied trash)",
    body: "Logical recovery. macOS doesn't immediately overwrite the data - it just marks the blocks free. As long as nothing has been written since, recovery is 90%+ successful, within 24 hours. Stop using the MacBook the moment you realise files are missing - every save shrinks the recovery window." },
  ...
  { title: "Water-damaged MacBook - data on the drive",
    body: "After ultrasonic cleaning, the SSD chips are read directly. That applies if the SSD is the only damaged component. If the secure enclave is corroded on T2/Apple Silicon, recovery becomes much harder - diagnosis tells you which scenario applies before any paid work." },
  { title: "T2 chip Mac (2018-2020 Intel) - board working but Mac won't boot",
    body: "T2 Macs encrypt the SSD with a key in the secure enclave. If the board still powers on, the workshop can boot into Apple Configurator restore mode and pull data via Target Disk Mode. 48-72 hours, 70% success rate." },
  { title: "Apple Silicon (M1/M2/M3) - secure enclave intact, won't boot",
    body: "M-series Macs have the SSD soldered directly to the logic board with the encryption key in the secure enclave. As long as the secure enclave chip itself is healthy, board-level repair restores access to data. Cost depends on what failed. 60% success rate, honestly stated." },
  { title: "External drive or Time Machine corrupted",
    body: "External drive recovery uses the same chip-off rig. Standard 2.5\"/3.5\" drives cost less than portable USB-C SSDs. Time Machine corrupted-backup recovery has a 95% success rate - the workshop reads the underlying APFS or HFS+ structure directly." },
  { title: "macOS won't update - disk error blocks installation",
    body: "Often a corrupted catalog file rather than a hardware failure. Free diagnosis runs Disk Utility First Aid then a deeper APFS check. If it's catalog corruption (50% of cases) the fix is free as part of diagnosis. If it's a failing SSD, recovery is quoted after diagnosis." },
  { title: "Folder shows but files appear empty (0 bytes)",
    body: "APFS metadata corruption. The file content is still on the drive but the inode pointers are broken. Custom recovery script reconstructs the pointers from the journal in 24 hours, 85% success rate." },
];
...
const STEPS = [
  ...
  { title: "Transfer to your new drive or cloud",
    body: "Delivered on a fresh external SSD (yours, or we supply a 1 TB drive) or uploaded to your iCloud / Google Drive / Dropbox via the workshop's gigabit line. Encrypted in transit." },
...
const FAQS: FAQ[] = [
  { q: "How much does MacBook data recovery cost in Dubai?",
    a: "It depends on the tier: logical recovery of deleted or formatted files, physical recovery on Intel SSDs, T2 chip recovery, Apple Silicon NAND recovery, or complex multi-chip board work. Diagnosis is free and the figure is confirmed after it - never quoted blind. Send us the model and what happened on WhatsApp." },
  ...
  { q: "Can you recover data from a T2 Mac (2018-2020)?",
    a: "Yes, if the logic board is alive enough to boot into Apple Configurator restore mode or Target Disk Mode. 70% success rate. If the T2 chip itself is dead, the AES encryption key is gone and the data is unrecoverable - same constraint applies at Apple. Free diagnosis confirms which scenario." },
  { q: "Can you recover data from M1, M2, M3, M4 MacBooks?",
    a: "Yes, with caveats. The SSD on Apple Silicon is soldered to the logic board and encrypted by the secure enclave. If the secure enclave chip is healthy and the failure is elsewhere on the board, component-level repair restores boot and data, at 60% success. If the secure enclave is dead, data is gone. Time Machine backups taken before failure are hugely valuable on Apple Silicon." },
  ...
  { q: "Can I get my files back from an emptied Trash?",
    a: "Usually yes within the first few days - macOS marks the blocks free but doesn't overwrite them immediately. The workshop's deleted-file scan finds files by their content signatures (magic numbers) rather than the file table. 24 hours, 90% success if you stop using the MacBook the moment you realise files are missing." },
  { q: "What about my photos in iCloud - do you handle that too?",
    a: "Yes - Time Machine restore plus iCloud Photo Library re-download is included in the migration service. The workshop signs into your iCloud on the new MacBook (with you present), waits for the photo download, and verifies the count matches your old library before handover." },
...
const COMPARISON = [
  ["Free diagnosis",                 "Yes - 30 min, written quote",            "AED 379 / AED 999 evaluation fee (Apple list)"],
  ["Logical recovery","Free diagnosis, then a quote",                        "Refused - refers to third party"],
  ["T2 recovery",     "Free diagnosis, then a quote",                        "Refused - refers to third party"],
  ["Apple Silicon recovery","Free diagnosis, then a quote",                  "Refused - refers to third party"],
  ...
  useSeo(
    {
      ...
      description:
        "MacBook data recovery Dubai. Dead board, water damage, T2 and Apple Silicon, deleted files. Free diagnosis. 60-90% recovery rates. 055 741 3706.",
  ...
  return (
    <PageShell>
      ...
      <Hero
        ...
        subtitle="Dead logic board, water-damaged SSD, deleted files, T2 and Apple Silicon NAND recovery. Free diagnosis, then a firm quote. Honest success rates published below."
        startingPrice={299}
        ...
        <div className="max-w-[820px] flex flex-col gap-3xl">
          ...
          <section>
            <LinkifyProse selfHref="/macbook-data-recovery-dubai"><p className="text-[17px] text-text leading-relaxed">
              MacBook data recovery in Dubai spans logical recovery (deleted files, formatted drives, corrupted folders) through to complex board-level multi-chip work on Apple Silicon. The workshop has recovered 1,200+ MacBooks since 2004 - Intel SSDs, T2-encrypted drives, M1/M2/M3 NAND chips, water-damaged blade SSDs. Free 30-minute diagnosis, honest success rates, and a file preview before any payment is taken. No recovery, no charge for the labour - only the genuine parts cost for forensic media, if any is used. T2 and Apple Silicon recovery has real limits because of secure-enclave encryption - the section below explains exactly what those limits are.
            </p></LinkifyProse>
          </section>
          ...
          <section className="bg-bg-card rounded-md p-lg border border-border">
            ...
            <ul className="space-y-2 text-[14px] text-text-muted">
              <li><strong className="text-text">If the secure enclave is alive</strong> - board-level repair restores boot and data. 60-70% success.</li>
          ...
          <section>
            ...
            <div className="border border-border/70 bg-bg-card ring-1 ring-black/[0.03] rounded-md p-lg">
              <ul className="space-y-2 text-[15px] text-text">
                <li><strong>No recovery, no labour charge.</strong> If the file preview is empty or doesn't include what you need, you pay nothing for the labour - only forensic media costs, if any are used.</li>
          ...
          <section>
            <RelatedServices tone="dark" items={[
              ...
              { label: "MacBook Water Damage Repair", href: "/macbook-water-damage-repair-dubai", description: "Liquid in the SSD area? Handled as a combo job: clean plus recovery." },
