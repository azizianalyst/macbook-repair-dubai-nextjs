...
const SIGNS: { Icon: LucideIcon; label: string; body: string }[] = [
  ...
  { Icon: AlertTriangle, label: "Dead or stuck pixels",   body: "Pixel cluster after a drop points to impact damage inside the bonded panel, so it is replaced as a unit." },
...
const PROBLEMS = [
  { title: "Cracked screen on M2, M3 or M4 Air — full panel only",
    body: "From the M2 Air (2022) onward the glass is bonded to the Liquid Retina panel as one assembly. There is no glass-only option — the full display unit must be replaced. Only the Intel 11\"/13\" and the M1 allow a glass-only repair on early production models." },
  { title: "Backlight failure on Intel Air",
    body: "Dark screen but image visible under a torch = backlight fault, not a panel. On Intel 11\"/13\" models the cause is usually the backlight IC or LED flex — board work saves you the cost of a full panel. Free diagnosis confirms which before we quote." },
  { title: "Screen flickering or pulsing brightness",
    body: "On M1 and M2 Air this is most often the display flex cable or connector working loose — a cable fix on M1. On Intel models a failing LVDS connector or voltage sag on the display rail causes the same symptom. If flicker changes with hinge angle it is the cable." },
  ...
  { title: "Screen lines — horizontal or vertical",
    body: "A line that changes position or disappears when you flex the lid is the display cable — cable replacement. A line fixed in position that appears on an external monitor too is a GPU fault. Lines fixed only on the built-in screen point to the panel, which is replaced as a unit." },
  ...
  { title: "Built-in display dead, external monitor works",
    body: "The external output is a separate circuit from the internal panel backlight. When only the built-in is dead, it is almost always the panel or the backlight IC — not the GPU. Around 75% of these are panel swaps; the rest are a backlight board repair, which costs less." },
  { title: "Dead or stuck pixels",
    body: "A single stuck pixel on a new panel counts as a warranty claim. If you have a dead-pixel cluster after a drop, the panel has impact damage, so it is replaced in full. We run an 8-colour dead-pixel scan before and after every repair so the count is documented on the warranty card." },
...
const FAQ_GROUPS = [
  {
    ...
    items: [
      { q: "How much does MacBook Air screen repair cost in Dubai?", a: "It depends on the model — the Intel 11-inch, the M1 13-inch, and the M2 through M4 panels are all different parts, and the 15-inch costs more than the 13-inch. Diagnosis is free and we quote your exact model before starting. All prices include the panel, labour, and warranty — no diagnostic fee. WhatsApp the model name or chip for the exact figure." },
      ...
      { q: "Do you offer free pickup for MacBook Air screen repair in Dubai?", a: "Yes — free pickup and delivery across Dubai mainland, including Marina, Downtown, JBR, JLT, Palm Jumeirah, Business Bay and Al Barsha. Same-hour pickup is available from Internet City and Knowledge Village. Pickup to Sharjah and Abu Dhabi carries a trip charge, confirmed before we dispatch." },
      { q: "How much is MacBook Air screen repair compared to the Apple Store?", a: "Apple charges from around AED 900–1,200 for an M1/M2 Air screen out of warranty (AED 1,100–1,900 for M3/M4), with a 5–14 business day wait and no pickup option. We quote after a free diagnosis, finish same day to 2 days, and offer free pickup across Dubai. Apple does not service Intel models it has declared vintage — we still repair them, quoted per model." },
    ],
  },
  {
...
const RELATED = [
  ...
  { label: "MacBook screen repair (all models)",     href: "/macbook-screen-repair-dubai",          description: "The full MacBook screen service — Air, Pro, Intel." },
  ...
  useSeo(
    {
      ...
      description:
        "MacBook Air screen repair Dubai. M1–M4 Liquid Retina, Intel IPS, True Tone, same-day. Genuine Apple panels, 3-month warranty. Call 055 741 3706.",
      ...
      <div className="-mb-[4rem] bg-bg text-text">
          ...
          >
            ...
            <div itemProp="acceptedAnswer" itemScope itemType="https://schema.org/Answer">
              <p className="text-[15px] text-text-muted leading-relaxed" itemProp="text">
                MacBook Repair Dubai replaces MacBook Air screens at its Concord Tower workshop in Dubai Media City. Intel IPS, M1 IPS Retina, and M2–M4 Liquid Retina panels in stock — same-day on most models, free diagnosis, free citywide pickup, genuine Apple panels for M3 and M4, written warranty up to 3 months.
              </p>
            </div>
          </div>
                ...
                {PRICING.map((r) => (
                  <tr key={r.model} className="border-b border-border last:border-0">
                    ...
                    <td className="px-lg py-sm whitespace-nowrap">
                      <PriceCTA compact message={`Hi, MacBook Air screen repair price for ${r.model}?`} />
                    </td>
        ...
        <section className="mx-auto max-w-content px-5 md:px-6 mt-[96px]">
          ...
          <div className="grid gap-x-xl gap-y-[48px] sm:grid-cols-2 lg:grid-cols-3">
            {[
              { title: "Original Apple (OEM)",  sub: "Costs more than A-grade",       body: "Genuine Apple Liquid Retina panels stocked for the M3 and M4 Air — sourced from the local Apple-authorised distributor with traceable batch numbers. True Tone, full P3 brightness retained. Only option that keeps True Tone active on M2, M3 and M4." },
              { title: "A-grade (Samsung / LG)", sub: "Our standard panel grade", body: "Same Samsung and LG factories that build the Apple panel — without the Apple calibration chip. Full resolution, full brightness, and P3 colour where the model supports it. True Tone is disabled by macOS on M2/M3/M4 aftermarket panels but everything else is full-spec." },
