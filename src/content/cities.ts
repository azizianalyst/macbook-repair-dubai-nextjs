...
const BASE_CITIES: CityConfig[] = [
  ...
  },
  {
    ...
    "whyChoose": "Most laptop shops on the east coast handle phones and generic PCs. We only do Apple, and only at one bench in Dubai, so the MacBook that leaves Khor Fakkan is worked on by someone who's seen the same fault hundreds of times. Board-level work, screen bonding, battery cells with the cycle count reset, none of that is sent on to a third party. It stays in our Media City workshop. The courier is free both ways, the diagnostic is free, and it's no-fix-no-charge, so the two-hour distance costs you nothing. You also get a price in writing before we touch anything. 232+ Google reviews, 5.0 stars, and the same QC sheet on every job.",
